...
__all__ = ['CESineCosineAlgorithm']


class CESineCosineAlgorithm(CoEvolutionOptimizationAlgorithm):
    r"""Implementation of sine cosine algorithm for coevolution.
    ...
    Name = ['CoEvolutionSineCosineAlgorithm', 'CE-SCA']
    ...
    def set_parameters(self, population_size=25, a=3, r_min=0, r_max=2, *args, **kwargs):
    ...
    def next_position(self, x, groups, best_x, r1, r2, r3, r4, task):
        ...
        """
        return task.repair(x[groups] + r1 * (np.sin(r2) if r4 < 0.5 else np.cos(r2)) * np.fabs(r3 * best_x[groups] - x[groups]), rng=self.rng)

    def run_iteration(self, task, groups, population, population_fitness, best_x, best_fitness, iters, **params):
        ...
        Args:
            ...
            iters (int): Current algorithm iteration.
        ...
        """
        r1 = self.a[groups] - (iters + 1) * (self.a[groups] / (iters + 1))
        r2 = self.uniform(0, 2 * np.pi)
        r3 = self.uniform(self.r_min, self.r_max)
        r4 = self.random()
        population = np.apply_along_axis(self.next_position, 1, population, groups, best_x, r1, r2, r3, r4, task)
